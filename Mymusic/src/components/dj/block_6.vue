<template>
	<div class="block_2_1">
			<div class="title_4">
				<span class="choose_carefully">音乐故事</span>
			</div>
			<div class="content">
				<div class="box">
					<div class="box_in"v-for="(box_item,box_index) in result" :key="box_index">
						<img :src="box_item.picUrl+'?param=50y50'">
						<i class="song">{{box_item.lastProgramName}}</i>
						<i class="name">{{box_item.name}}</i>
...
<script>
	export default{
		...
		mounted(){
			var url = this.HOST + '/dj/recommend/type?type=2';
			this.$axios.get(url)
			.then(res => {
				var temp = res.data.djRadios;
				for(var i = 0; i<9;i++){
