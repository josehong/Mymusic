<template>
	<div class="album">
		<div class="title"><span @click="goback"><=专辑</span></div>
		<div class="null"></div>
		<div class="background">
				<img :src="listname.picUrl+'?param=100y100'" />
				<p class="name">{{listname.name}}</p>
				<!--<p class="author">{{listname.artists[0].name}}</p>-->
	...
	export default{
		...
		mounted(){
			if(this.$route.params.id != null){
				...
				var url = this.HOST+'/album?id='+this.$store.state.playlistid
				this.$axios.get(url)
				.then(res =>{
					
						
						this.songlist = res.data.songs
						this.listname = res.data.album
...
<style>
	...
	.album .box .singerlist_in{
		width: 300px;
		height: 50px;
		margin: 5px 0 0 10px;
		/*background: red;*/
	}	
	.album .box .singerlist_in span{
		width: 16px;
		height: 16px;
		text-align: right;
		display: inline-block;
	...
	.album .box .singerlist_in .content{
		float: left;
	}
	.album .box .singerlist_in p{
		width: 235px;
		height: 15px;	
		overflow: hidden;
		/*font-size: 10px;*/
	...
	.album .box .singerlist_in .sing{
		font-size: 12px;
	}
	.album .box .singerlist_in .name{
		font-size: 10px;
	}
</style>