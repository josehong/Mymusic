<template>
	<div class="block_1">
			...
			<div class="content">
				<div class="box">
					<router-link tag="div" :to="{name:'Secondsonglist',params:{id:box_item.id}}" class="box_in" v-for="(box_item,box_index) in result" :key="box_index">
...
<script>
	export default{
		...
		mounted(){
			var url = this.HOST + '/personalized?limit=6';
			this.$axios.get(url)
			.then(res => {
...
<style>
	...
	.block_1 .recommend_list{
		font-size: 10px;
		display: block;
		margin-left: 10px;
	}
	.block_1 .choose_carefully{
		height: 22px;
		line-height: 25px;
		text-align: center;
		font-size: 14px;
	...
	.block_1 .content::-webkit-scrollbar {
        display: none;
    }
	.block_1 .box{
		width: 665px;
